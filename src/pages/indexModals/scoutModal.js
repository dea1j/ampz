...
const ScoutModal = ({ formFour, setFormFour }) => {
  ...
  const { fullname, organization, email, phone, interest } = data;
  const handleSubmit = async (e) => {
    e.preventDefault();

    try {
      const response = await fetch(
        "https://v1.nocodeapi.com/dea1j/google_sheets/JbWynJoPzkGmkcLy?tabId=Sheet1",
        {
          method: "post",
          body: JSON.stringify([
  ...
  return (
    <Modal show={formFour} onHide={() => closeForm()}>
